...
export class BookingServiceStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: BookingServiceStackProps) {
    ...
    const cancelbookingFunction = new lambdaNodejs.NodejsFunction(this, 'dancelbookingFunction', {
      functionName: `cancelbookingFunction-${stage}`,
      entry: 'LambdaFunctions/Booking/cancelBooking.js',
      runtime: lambda.Runtime.NODEJS_20_X,
      handler: 'handler',
      depsLockFilePath:'package-lock.json',
      environment: {
    ...
    bookingTable.grantReadWriteData(cancelbookingFunction);
    ...
    apiGatewayRole.addToPolicy(new iam.PolicyStatement({
      ...
      resources: [createbookingFunction.functionArn, getbookingFunction.functionArn, updatebookingFunction.functionArn, cancelbookingFunction.functionArn],
    }));
    ...
    const cancelbookingIntegration = new apigateway.LambdaIntegration(cancelbookingFunction);
    ...
    bookingIdResouce.addMethod('CANCEL', cancelbookingIntegration); // CANCEL /booking/{bookingId}
